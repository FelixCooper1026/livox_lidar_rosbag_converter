...
inline std::vector<sensor_msgs::PointField> MakeLivoxPointFields() {
  std::vector<sensor_msgs::PointField> fields(7);
  ...
  fields[4].name = "tag";
  fields[4].offset = 16;
  fields[4].datatype = sensor_msgs::PointField::UINT8;
  fields[4].count = 1;

  fields[5].name = "line";
  fields[5].offset = 17;
  fields[5].datatype = sensor_msgs::PointField::UINT8;
  fields[5].count = 1;

  fields[6].name = "timestamp";
  fields[6].offset = 18;
  fields[6].datatype = sensor_msgs::PointField::FLOAT64;
  fields[6].count = 1;
...
inline std::string DescribePointField(const sensor_msgs::PointField& field) {
...
inline std::string DescribePointFieldsMultiline(const std::vector<sensor_msgs::PointField>& fields,
  ...
  for (const sensor_msgs::PointField& field : fields) {
...
inline std::string DescribePointCloud2Format(const sensor_msgs::PointCloud2& msg) {
  const size_t point_count = static_cast<size_t>(msg.width) * static_cast<size_t>(msg.height);
  std::ostringstream stream;
  stream << "PointCloud2"
         << "\n    frame_id: " << msg.header.frame_id
         << "\n    stamp_ns: " << msg.header.stamp.toNSec()
         << "\n    height: " << msg.height
         << "\n    width: " << msg.width
         << "\n    points: " << point_count
...
inline std::string DescribePointFieldDifferences(const sensor_msgs::PointCloud2& msg) {
  const std::vector<sensor_msgs::PointField> expected_fields = MakeLivoxPointFields();
...
inline std::string FormatMismatchMessage(const std::string& summary,
...
inline std::string DescribeCustomMsg(const livox_ros_driver2::CustomMsg& msg) {
  std::ostringstream stream;
...
inline void RequireLivoxPointCloud2Format(const sensor_msgs::PointCloud2& msg) {
  const std::vector<sensor_msgs::PointField> fields = MakeLivoxPointFields();
  ...
  for (size_t i = 0; i < fields.size(); ++i) {
    if (msg.fields[i].name != fields[i].name ||
      ...
      throw std::runtime_error(FormatMismatchMessage(
          "PointCloud2 field layout does not match Livox PointXYZRTLT", msg));
    }
  }
  ...
  if (msg.data.size() < point_count * sizeof(LivoxPointXyzrtlt)) {
    throw std::runtime_error(FormatMismatchMessage(
        "PointCloud2 data is shorter than Livox PointXYZRTLT point count", msg));
  }
}

inline sensor_msgs::PointCloud2 ToPointCloud2(const livox_ros_driver2::CustomMsg& msg) {
  if (msg.point_num != msg.points.size()) {
    throw std::runtime_error("CustomMsg point_num does not match points array size\n  actual: " +
                             DescribeCustomMsg(msg));
  }
  ...
  for (size_t i = 0; i < msg.points.size(); ++i) {
    const livox_ros_driver2::CustomPoint& src = msg.points[i];
...
inline livox_ros_driver2::CustomMsg ToCustomMsg(const sensor_msgs::PointCloud2& msg) {
  RequireLivoxPointCloud2Format(msg);

  const size_t point_count = static_cast<size_t>(msg.width) * static_cast<size_t>(msg.height);
  const uint64_t timebase = msg.header.stamp.toNSec();
  ...
  for (size_t i = 0; i < point_count; ++i) {
    ...
    livox_ros_driver2::CustomPoint& dst = custom_msg.points[i];
    dst.x = src.x;
    dst.y = src.y;
    dst.z = src.z;
    ...
    dst.tag = src.tag;
    dst.line = src.line;
    dst.offset_time = static_cast<uint32_t>(
        std::llround(src.timestamp - static_cast<double>(timebase)));
  }
